...
import type {
  ...
  IStatusCodeMessages,
  Functionality
} from "src/interfaces/generic";
import type { INode } from "src/interfaces/node";
import { NodeError } from "./abstract/node.error";
import { removeCircularRefs } from "src/utilities/helpers";
  ...
  constructor(
    ...
    if (
      ...
      (errorResponse.description ||
        (errorResponse?.reason as IDataObject)?.description)
    ) {
      // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
    ...
    if (errorResponse.reason) {
      const reason: IDataObject =
        errorResponse.reason as unknown as IDataObject;
  ...
  private setDefaultStatusCodeMessage() {
    ...
    switch (this.httpCode.charAt(0)) {
      ...
      default:
        // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
        this.message =
          this.message || this.description || UNKNOWN_ERROR_MESSAGE;
    }
    if (
      this.node.type === "n8n-nodes-base.noOp" &&

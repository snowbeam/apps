import type { IDataObject } from "src/interfaces/generic";

const defaultPropertyDescriptor = Object.freeze({
  enumerable: true,
  configurable: true
});
...
export function augmentArray<T>(data: T[]): T[] {
  ...
  const proxy = new Proxy(data, {
    ...
    getOwnPropertyDescriptor(target, key) {
      ...
      return (
        Object.getOwnPropertyDescriptor(data, key) ?? defaultPropertyDescriptor
      );
    },
    has(target, key) {
